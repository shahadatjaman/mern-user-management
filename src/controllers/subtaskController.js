...
exports.createSubtask = async (req, res) => {
  try {
    const { task, title, createdBy } = req.body;
    const taskObjId = new Types.ObjectId(task);
...
exports.getSubtasksByTask = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch subtasks", error: err.message });
  }
};
...
exports.updateSubtask = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to update subtask", error: err.message });
  }
};
...
exports.deleteSubtask = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to delete subtask", error: err.message });
  }
};
